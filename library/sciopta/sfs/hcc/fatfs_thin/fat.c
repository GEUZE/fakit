...
unsigned char ret;
      ...
      if (gl_volume.mediatype!=F_FAT12_MEDIA)
      {
        unsigned long sector, act_sector;
        unsigned short cpss;
        unsigned short ncl,i,j;
        unsigned char *fatbuf;

        if (gl_volume.mediatype==F_FAT16_MEDIA) cpss=8;
                                           else cpss=7;
        sector=(gl_volume.lastalloccluster)>>cpss;
        act_sector=sector;
        do {
          if (_GET_BIT(gl_volume.fatbitfield,act_sector)==0)
          {
            ret=_f_getfatsector(act_sector);
            if (ret) return ret;
            fatbuf=gl_volume.fat;

            ncl=(unsigned short)(1<<cpss);
            if (act_sector!=sector) 
            {
              j=0;
            }
            else
            {
              j=(unsigned short)((gl_volume.lastalloccluster)&(ncl-1));
              if (cpss==8) fatbuf+=2*j;
                      else fatbuf+=4*j;
            }

            if (act_sector==gl_volume.firstfat.num-1) ncl=(unsigned short)(gl_volume.maxcluster&(ncl-1));
            for (i=0;i<ncl;i++)
            {
              if (cpss==8)
              {
                if (_f_getword(fatbuf)==F_CLUSTER_FREE) break;
                fatbuf+=2;
              }
              else
              {
                if (_f_getlong(fatbuf)==F_CLUSTER_FREE) break;
                fatbuf+=4;
              }
              if (++j==ncl) 
              {
            ...
            if (i<ncl)
            {
              *pcluster=(act_sector<<cpss)+j;
              gl_volume.lastalloccluster=*pcluster;
              if (gl_volume.cspace_valid)
              {
                ++gl_volume.cspace.used;
                --gl_volume.cspace.free;
              }
              return F_NO_ERROR;
            }

            _SET_BIT(gl_volume.fatbitfield,act_sector);
          }
          if (++act_sector==gl_volume.firstfat.num) act_sector=0;
        } while (act_sector!=sector);
        
        return F_ERR_NOMOREENTRY;
      }
#endif

#if FATBITFIELD_ENABLE==0 || (FATBITFIELD_ENABLE && (F_FAT12))
	{
	  unsigned long value;
...
unsigned char _f_removechain(unsigned long cluster) {
	...
	while (cluster<F_CLUSTER_RESERVED && cluster>=2) {
		...
		if (ret) return ret;
#if FATBITFIELD_ENABLE && (F_FAT16 || F_FAT32)
		_CLEAR_BIT(gl_volume.fatbitfield,gl_volume.fatsector);
#endif
#if F_GETFREESPACE_QUICK || (FATBITFIELD_ENABLE && (F_FAT16 || F_FAT32))
		if (gl_volume.cspace_valid)
		{
