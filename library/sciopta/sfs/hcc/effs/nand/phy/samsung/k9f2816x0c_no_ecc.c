#ifndef _K9F2816X0C_no_ecc_C_
#define _K9F2816X0C_no_ecc_C_
...
static int ReadPage(unsigned char *dest,long pagenum) 
{
	...
	if (NANDDATA!=0xffffffffUL) return 0;
...
#else
	a=PAGE_SIZE/16;   /* write one page data */
	{
		register volatile unsigned long* nanddata=(unsigned long*)0x1000008;
		do 
		{
   			*nanddata= *rp++;
   			*nanddata= *rp++;
...
/****************************************************************************
 *
 *  end of K9F2816X0C_no_ecc.c
 *
 ***************************************************************************/

#endif /* _K9F2816X0C_no_ecc_C_ */
