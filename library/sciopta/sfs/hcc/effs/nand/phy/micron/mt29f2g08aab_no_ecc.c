#ifndef _mt29f2g08aab_no_ecc_C_
#define _mt29f2g08aab_no_ecc_C_
 ...
 * ReadPage
 *
 * Read one page from device to page buffer
 *
 * INPUTS
...
static int ReadPage(unsigned char *dest,long pagenum) 
{
	...
	b=NANDDATAL;  /* get bad block info as dummy */
	b=NANDDATAL;  /* get ecc from 1st 32bit in spare area */

   	for (a=0; a<PAGE_SIZE; a++) 
	{
	   	if (dest[a]!=(unsigned char)0xff) return 0;
   	}

   	if ((unsigned long)b==0xffffffffUL) return -1; /* erased not written */
...
static int WritePage(void *data, long pagenum, long size,long sdata) 
{
	...
	NANDDATAL = 0xf0f0f0f0;   /* write fake ecc also, but this is in spare area yet */
	NANDDATAL = sdata; /* add signature data into spare */
...
int fs_phy_nand_mt29f2g08aab(FS_FLASH *flash) 
{
	long manID, devID;
...
/****************************************************************************
 *
 *  end of mt29f2g08aab_no_ecc.c
 *
 ***************************************************************************/

#endif /* _mt29f2g08aab_no_ecc_C_ */
