...
#include <hcc/effs/nand/nflshdrv.h>
...
/****************************************************************************
 *
 * Some defintions for signature data
 *
 ***************************************************************************/
 ...
 * useable block, then Copy block into a new block with number of sectors.
...
static int CopyBB(FS_VOLUMEINFO *vi, int sectornum)
{
	...
	for (;;)
	{
		...
		/* set all sector in that block to not useable (bad) */
		for (a=0; a<vi->flash->sectorperblock; a++)
		{
		...
		if (!vi->flash->EraseFlash (vi->flash->blockstart+freeblock))
		{
			...
			/* copy all the sectors to the bew free block */
	...
	*vi->version=F_DRVVERSION;
	...
	for (;;) 
	{
		...
		if (currdesc!=-1) 
		{
			...
			{ 
				...
				for (a=0; a<vi->flash->maxblock; a++) 
				{
					if (vi->_blockindex[a]==(unsigned short)currdesc) 
					{
						...
						(void)(vi->flash->EraseFlash(currdesc+vi->flash->blockstart)); 
			...
			(void)(vi->flash->EraseFlash(currdesc+vi->flash->blockstart)); /* remove signatures and datas	from old FAT */
		}
		...
		/* any error so doing bad block handling, new block allocation */
		{
			...
			for (a=0; a<vi->flash->sectorperblock; a++) 
			{
				...
				_fsm_cacheaddsptr(vi,&vi->_fat[sector+a],FS_FAT_NOTUSED);	   
			}
...
static int GetSector (const FS_VOLUMEINFO *vi,long secnum,void *data,long offset,long datalen) 
{
	...
	{
		/* check wheter sector is used or not in mirror block */
	...
	blockrel=blockrel*vi->sectorsize+offset; /* calculate block relativ address */
...
/* check phy function */
	...
	{
		vd->state=FS_VOL_DRVERROR;	/* theese functions has to be implemented! */
		return 1;
	}
	...
	if (vi->flash->WriteVerifyPage)
	{
		...
		{
			vi->cache.desc=(hcc_u32 *)fsm_allocdata(vi,vi->flash->blocksize);
			if (!vi->cache.desc) 
			{
				vd->state=FS_VOL_NOMEMORY;
				return 1;
...
/* alloc copybits, copy bits is for determinating if the next sector space is a copyable sector only */
...
/* alloc desc (fat+directory and desc) */
	...
	else
	{
		...
		vi->_wearlevel=(hcc_32*)fsm_allocdata(vi,(vi->flash->maxblock+1)*(long)sizeof(hcc_32));
		if (!vi->_wearlevel)
		{
			vd->state=FS_VOL_NOMEMORY;
			return 1;
