...
static const char OEM_name[]="HCC_SAFE_FS";
...
int _fg_setdename(const W_CHAR *s, FS_VOLUMEINFO *vi, FS_DIRENTRY *de) 
{
	...
	if (clen>0) return 1; /* not enought space */
...
#if 0
		/* Set this #if to 1 if start spaces needs to be removed */
 ...
 * _fg_findpath
 *
 * finding out if path is valid and retrive directory number in FS_NAME and
 ...
 * offset - relativ position in sector from zero
...
int fg_wmkdir(FS_MULTI *fm,const W_CHAR *dirname) 
{
	...
	if (_fg_addentry(vi,&fsname,&de)) return F_SETLASTERROR(F_ERR_NOMOREENTRY); /* couldnt be added */
...
int _fg_fseek(FS_VOLUMEINFO *vi,FS_MULTI *fm,FS_FILEINT *f, long offset) 
{
	...
	while (offset) 
	{
		...
		/* check eof or invalid sector 2008.05.09 */
		if (nextsector>=FS_FAT_EOF) 
		{
			return F_ERR_ONDRIVE;	
		}
 ...
 * remove - signal aff appsync must be disconected
 *
 * RETURNS
 *
...
FS_FILE *fg_wopen(FS_MULTI *fm,const W_CHAR *filename,const W_CHAR *mode) 
{
	...
	for (a=0; a<vi->maxfile; a++) 
	{
		...
		{
			f=&vi->files[a];
			f->mode = FS_FILE_LOCKED; /* avoiding reentrancy for allocation */
			break;
		}
	}
	...
	switch (m_mode) 
	{
		...
		case FS_FILE_RDP: /* rp */
			...
			/* check wether there is an open appended file */
	   	...
	   	case FS_FILE_AP: /* a+ */
			if (_fg_find(vi,&fsname,&de,0))  
			{
				...
				f->direntry=de; /* 2008.05.08 de must be set before calling _fg_checkapwithr */
				...
				{
					/* f->direntry=de; */ /* de is already set 2008.05.08 */
					f->sectorstart=de->sector;
					f->len=de->len;
				}
 ...
 * last changings (from open or from the last flush operation) will be aborted
 *
 * INPUTS
 *
 * fm - multi structure pointer
