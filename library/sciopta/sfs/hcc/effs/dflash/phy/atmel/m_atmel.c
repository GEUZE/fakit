...
static long adf_blocks;
static long adf_block_start;
...
static adf_address_t adf_get_block_addr(long block, long relsector) {
   block-=adf_block_start;

   adf_var=&adf_vars[block/adf_blocks]; /* set up adf pointer */
   block=block%adf_blocks;              /* use only remaining numbers */

   return (adf_address_t)((F_ATMEL_BLOCKSIZE*(unsigned long)(adf_block_start+block))+((unsigned long)relsector*F_ATMEL_SECTORSIZE));
}
...
static int adf_read_flash(void *data, long block, long blockrel, long datalen)
{
	...
	adf_page_address_t page_addr=
		(adf_page_address_t)((adf_get_block_addr(block,0)+(unsigned long)blockrel)/ADF_PAGE_SIZE);
...
static int adf_erase_flash(long block)
{
	unsigned int x,y;
	adf_page_address_t page_addr=
		(adf_page_address_t)(adf_get_block_addr(block,0)/ADF_PAGE_SIZE);
...
int m_atmel_flash_fs_phy(FS_FLASH *flash)
{
	int a;
...
/*	fnPr("ADF maxblock pre: %d\n",flash->maxblock);*/

	adf_block_start=flash->descblockstart; /* set up relativ shifting */
	adf_blocks=flash->maxblock+F_ATMEL_NO_OF_DESC_BLOCKS;  /* calculate maximum available blocks */
	flash->maxblock+=(F_ATMEL_NO_OF_DEVICES-1)*adf_blocks; /* add device blocks to all */

/*	fnPr("ADF adf_blocks: %d\n",adf_blocks); */
/*	fnPr("ADF maxblock: %d\n",flash->maxblock); */
/*	fnPr("ADF adf_block_start: %d\n",adf_block_start); */
...
#if ADF_2_MAP_SECTOR
    assert(adf_var->adf_mgm_sel == 0 && (ctr_ofs >= PAGE2BYTEOFS(SECTOR2PAGE(ADF_LAST_0BMP_SECTOR+1)-1)+1)
       ||adf_var->adf_mgm_sel == 1 && (ctr_ofs >= PAGE2BYTEOFS(ADF_REAL_PAGE_COUNT)));
#else
