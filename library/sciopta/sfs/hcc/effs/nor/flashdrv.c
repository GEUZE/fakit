...
#include <hcc/effs/nor/flashdrv.h>
 ...
 * _chk_preerase
 *
 * checking if preerase is activ and set preeraseable descriptor block(s)
 *
 * INPUTS
 *
...
static int GetFat(FS_VOLUMEINFO *vi) 
{
	...
	if (vi->flash->cacheddescsize) {
		...
		if (vi->flash->ReadFlash(vi->cache.desc,vi->flash->descblockstart+currfat,vi->flash->descsize,vi->flash->cacheddescsize)) return 1; /* cant useable */
...
/* alloc desc (fat+directory and desc) */
	...
	if (vi->flash->cacheddescsize) 
	{
		vi->cache.desc=(hcc_u32 *)fsm_allocdata(vi,vi->flash->cacheddescsize);
		if (!vi->cache.desc) 
		{
			vd->state=FS_VOL_NOMEMORY;
			return 1;
