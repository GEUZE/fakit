...
INLINE_STATIC void fnWriteWord(volatile unsigned short* base,long addr,unsigned short data)
{
 ...
 *
 ***************************************************************************/
INLINE_STATIC void WriteCmd(unsigned short *base,unsigned short mit) {
  fnWriteWord(base,0x555<<1,_xAA);
  fnWriteWord(base,0x2AA<<1,_x55);
  fnWriteWord(base,0x555<<1,mit);
}
...
static int EraseFlash(long block) 
{
  ...
  base = g_pwflashbase + ((addr & ~(FLASH_SIZE-1))/2);
...
static int WriteFlash(void *data, long block, long relsector, long size,long relpos) 
{
  ...
  base = g_pwflashbase + ((dest & ~(FLASH_SIZE-1)))/2;
...
{
  long addr=g_pwflashbase; /* get the flash base address */
 ...
 *
 ***************************************************************************/
int fs_phy_nor_29gl128p(FS_FLASH *flash) 
{
  ...
  WriteCmd(g_pwflashbase,_xF0); /* Exit this mode, reset state machine in the flash */
  
  WriteCmd(g_pwflashbase,_x90);           /* ManID manufacture ID */
  man_ID=g_pwflashbase[0x00];
  
  WriteCmd(g_pwflashbase,_x90);           /* DevID Device ID */
  dev_ID=g_pwflashbase[0x01];
  
  if(dev_ID==0x227e)
  {
    dev_ID1=g_pwflashbase[g_pwflashbase,0x0e];
    dev_ID2=g_pwflashbase[g_pwflashbase,0x0f];
  }
    
  WriteCmd(g_pwflashbase,_xF0);           /* Exit this mode */
  
  SetWriteProtect();
  
  if ((man_ID & 0xff)==0x01 && 
      dev_ID==0x227e && 
      dev_ID1==0x2221 /* LG128 */ && 
      dev_ID2==0x2201)
  {
