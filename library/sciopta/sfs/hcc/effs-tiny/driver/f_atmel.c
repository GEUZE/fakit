...
#ifndef TINY_USED_ADDR_FN
#define TINY_USED_ADDR_FN 0
#endif
/*****************************************************************************
 *
 * Type definitions.
...
unsigned char adf_copy (adf_address_t dstflashaddr, adf_address_t srcflashaddr, unsigned int size)
{
  ...
  while (size != 0)
  {
    if (adf_transfer_buff_1(src_page_addr) != 0)
    {
      return(1);
    }
...
unsigned char adf_erase_safe(adf_address_t flashaddr, unsigned int size)
{
  ...
  if (adf_lock_page(safe, PAGE_ADDRESS_MSB | (PAGE_ADDRESS_MSB>>1)| page_addr))
  {
    return(1);
  }
  adf_cmd(ADF_ERASE_PAGE, PGA2FLA(page_addr));
 ...
 * 1: othervise
 ***************************************************************************/
unsigned char adf_write (adf_address_t flashaddr,void *ramaddr, unsigned int size)
{
  adf_page_address_t page_addr;
  adf_byte_address_t byte_addr;
  adf_byte_address_t write;
  unsigned char *src=(unsigned char *)ramaddr;
 ...
 * 1: othervise
 *******************************************************************************/
unsigned char adf_write_safe (adf_address_t flashaddr, void *ramaddr, unsigned int size)
{
  adf_page_address_t page_addr;
  adf_byte_address_t byte_addr;
  const unsigned char *src=(unsigned char *)ramaddr;
  ...
  if (size)
  {
    ...
    if (adf_lock_page(safe, PAGE_ADDRESS_MSB | page_addr) != 0)
    {
      return(1);
    }
    /* Do the transaction. */
  ...
  if (adf_update_ctr[sector]++ >= (((10000u/2u)/SECTORSIZE(sector))-1))
  {
    ...
    if (x_bit)
    {
...
#endif
      {
        ...
        /* Log transatcion start. */
        if (adf_lock_page(safe, PAGE_ADDRESS_MSB | pg_ndx) != 0)
        {
          return(1);
        }
        /* Do the transaction. */
