...
#endif
#ifndef FN_MAXTASK
#define FN_MAXTASK    2       /* maximum number of task */
#endif

#define FN_MAXPATH    256

  /* setting the current drive at startup (-1 means no default current drive)*/
#define FN_CURRDRIVE	0
...
#ifndef SC_MALLOC_MSGID
#define SC_MALLOC_MSGID 0x11223344
#endif

#define __malloc(a) sc_malloc(a)
#define __free(a) sc_free(a)

STATIC_INLINE void * sc_malloc(size_t size)
{
  void *tmp;
  if ( size == 0 ){
    return NULL;
  ...
  tmp = sc_msgAlloc (size + sizeof (sc_msgid_t),
		     HCC_FATFS_MALLOC,
		     SC_DEFAULT_POOL,
		     SC_NO_TMO);
  if (!tmp) {
...
STATIC_INLINE void sc_free(void *ptr)
{
  if (ptr) {
