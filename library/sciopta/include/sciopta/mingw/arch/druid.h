 ...
 * Global definitions
 */
#define sciopta _sciopta
#define sc_modules _sc_modules
#define sc_connectors _sc_connectors
...
#define DRUID_PROC_CB(module, process) \
  ((__u32 ) ((__u8 *) sc_modules[module]->pcbs[process]))
...
STATIC_INLINE __u32 nObserve(sc_pcb_t * pcb)
{
  __u32 n = 0;
...
static INLINE __u32 bufsizeIdx(sc_msg_hd_t *msghd)
{
  if ( (msghd->plid & 0x00ffff00) == 0x00affe00 ){
    return msghd->size >> 28;
  } else {
    return msghd->bufsize_idx;
  }
}

static INLINE __u32 msgOwner(sc_msg_hd_t *msghd)
{
  if ( (msghd->plid & 0x00ffff00) == 0x00affe00 ){
    return 0;
  } else {
...
static INLINE __u32 msgAddr(sc_msg_hd_t *msghd)
{
  if ( (msghd->plid & 0x00ffff00) == 0x00affe00 ){
    return 0;
  } else {
...
#define DRUID_MSG_SIZE(msg) \
  (DRUID_MSG_HEADER (msg)->size & 0x0fffffff)

#define DRUID_MSG_PLID(msg) \
  (DRUID_MSG_HEADER (msg)->plid & 0xff0000ff)

#define DRUID_MSG_IS_TMOMSG(msg) \
  ((DRUID_MSG_HEADER (msg)->plid & MSG_TMO_MASK) == MSG_TMO_MAGIC)

#define DRUID_MSG_OWNER(msg) msgOwner(DRUID_MSG_HEADER (msg))

#define DRUID_MSG_SENDER(msg) \
  DRUID_MSG_HEADER (msg)->sender 

#define DRUID_MSG_ADDRESSEE(msg) msgAddr(DRUID_MSG_HEADER (msg))
