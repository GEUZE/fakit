...
** ID: S04021CL1
** +Revision: 1.14 +
** +Date: 2009/01/23 16:15:03 +
** CRC claculation (X16 + X12 + X5 + 1)
**********************************************************************
EOC*/
...
{
  return sc_miscChksumContd(dp,count,0);
}

__u16 sc_miscChksumContd (const void FARPTR dp, __u16 count,__u16 total_init)
{
  const __u16 FARPTR p;
  __u32 total = (__u32)total_init;
  p = (const __u16 FARPTR)dp;
  while (count > 1) {
    total += *p++;
    count -= 2;
  }
  ...
  if (count) {
    total += (*(const __u8 FARPTR) p) << 8;
  }
#elif (__BYTE_ORDER == __LITTLE_ENDIAN)
  if (count) {
    total += *(const __u8 FARPTR) p;
  }
#else
...
{
  while( len ){
    hash = sc_miscCrcTab[(__u8)hash ^ *data ] ^ (hash>>8);
    --len;
    ++data;
  }
  return (__u16)hash;
...
{
  return sc_miscCrcContd(data,len,(__u16)0xffff);
}
...
__u16 sc_miscCrcString(const char FARPTR data)
{
  ...
  char c;
  while( (c = *data) ){
    hash = sc_miscCrcTab[(__u8)hash ^ c ] ^ (hash>>8);
    ++data;
  }
  return hash;
}
