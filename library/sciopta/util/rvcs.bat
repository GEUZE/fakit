...
set INC=-I. -J%SCIOPTA_HOME%\include -I%SCIOPTA_HOME%\include\sciopta\arm -J"%RVCT21INC%"

set CFLAGS= -Wa -apcs /interwork -zo -Wb -g
set ARFLAGS= --create
REM --------------
REM Library names
REM --------------
...
%CC% -c %CFLAGS% %OSIZE% %INC% version_util.c
%AR% %ARFLAGS% ..\lib\arm\arm\rvct21\%LIB% version_util.o
...
%AR% %ARFLAGS% ..\lib\arm\arm\rvct21\%LIBSH% version_sh.o
echo Building in SH
pushd sh
set L=
for %%o in (*.c) DO (
...
echo Compiling: !L!
%CC% -c %CFLAGS% %OPT% %INC% !L!
%AR% -r ..\..\lib\arm\arm\rvct21\%LIBSH% *.o
del *.o
popd
