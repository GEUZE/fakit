...
static int mst_connect (void *dev_hdl, void *ifc_hdl)
{
  ...
  if (mi->ep_hdl[EP0] && mi->ep_hdl[IN] && mi->ep_hdl[OUT])
  {
    ...
    if (scsi_open_device((hcc_u8)(mi-mst_info),mi->nlun,usb_mst_transfer,dev_hdl)==0)
    {
      mi->mst_device=1;
      mi->ifc_hdl=ifc_hdl;
...
{
  unsigned int i;
  sc_pid_t self = sc_procIdGet(NULL,SC_NO_TMO);
  for (i=0;i<SCSI_MAX_DEVICE;++i){
    mst_info[i].mst_device = 0;
    /* SCIOPTA */
    mst_info[i].device = i;
    mst_info[i].driver = self;
  }
  scsi_init();
