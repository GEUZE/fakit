	...
	if (cfc->changed)
	{
		...
		if (!(state&F_ST_MISSING))
		{
...

			{
				...
				for (i=0;i<50000;) i++; /* wait some ms */
			}
   			CFC_VAL(CFC_CONF)=2;		/* set CFC to work in I/O mode at 1F0h */
...
static long fnCFCsetCommand (const t_CfcDrv *cfc,unsigned long dwcmd, unsigned char top4)
{
	CFC_VAL(CFC_CMDSELC)=(((unsigned char)dwcmd)<<8)+CFC_SELCARD(p)+top4;    /* command + select card */
