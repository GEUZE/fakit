...
{
	unsigned short PIO_cycle[]={600,383,240,180,120};
	...
	HDD_CSADDR0=0xb000;
	HDD_CSOPT0=0x3d00;
	HDD_CSMASK0=0xffff0001;
#else
	/*HDD_CSOPT0=0xfffff078;*/
	HDD_CSOPT0=0xfffff034;
	HDD_CSBASE0=0xb0000001;
#endif

	HDD_CONTROL0=0x00000000;	/* put to reset state */
	{
		volatile unsigned long i2;
		for (i2=0;i2<HDD_INIT_TO;i2++);
	}
	HDD_CONTROL0=0x00010000;	/* put to working state */
	{
		volatile unsigned long i2;
		for (i2=0;i2<HDD_INIT_TO;i2++);
	}
	...
	if ((get16(gl_data+49*2)&(1<<9))==0)
	{
		return 0;
	}

	/* set the quickest PIO mode without flow control */
	min_cycle=get16(gl_data+67*2); /* get minimum allowed PIO cycle */
	for (i=0;i<((sizeof(PIO_cycle)/sizeof(unsigned short))-1) && min_cycle<PIO_cycle[i+1];i++);
	HDD_VAL(hdd,HDD_FEATURE)=i<<16;       /* set PIO mode */
	HDD_VAL(hdd,HDD_SECTORCOU)=0x000b0000;
	HDD_VAL(hdd,HDD_SELC)=HDD_SELCARD(hdd);
	HDD_VAL(hdd,HDD_COMMAND)=HDD_SETFEA;
	if (fnHDDwaitBusy(HDD_PTR(hdd,HDD_STATE))) return 0;
#ifdef HCC_HW
#ifdef FAT_CONF_5282
	HDD_CSOPT0=0x2500;
#else
	HDD_CSOPT0=0xfffff024;	/* change MCF CS for PIO3 timing */
#endif
#endif
