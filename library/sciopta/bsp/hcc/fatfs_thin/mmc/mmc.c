...
unsigned long tNAC=0;			/* READ: response to data time (varies upon card) */
unsigned long tBUSY=0;			/* WRITE: maximum write time */
...
t_mmc_dsc mmc_dsc;		/* current MMC/SD card descriptor */
 ...
 ***************************************************************************/
#if ASM_OPT_8051
unsigned char mmc_cmd(unsigned char, unsigned long);
#else
unsigned char mmc_cmd(unsigned char cmd,unsigned long _data) 
{
   unsigned char a;
   unsigned char tmp=0;
 ...
 #endif
   {
     ...
     {     /* send start and cmd */
       ...
       cmd<<=1;
       tmp<<=1;
       if (tmp&0x80) tmp^=0x09;
     }
   }
#endif

   spi_tx1(_data>>24);  /* send data */
   spi_tx1(_data>>16);
   spi_tx1(_data>>8);
   spi_tx1(_data);
 ...
 * sector - address where to start read card, it has to be sector aligned
 *
 * RETURNS
 *
 * 0 - if successful
 ...
 ***************************************************************************/
#if ASM_OPT_8051==0
unsigned char mmc_readsector (unsigned char *dwaddr,unsigned long sector) 
{
#if USE_CRC || SDHC_ENABLE
   ...
   sector<<=9;
#endif
   r1=mmc_cmd(CMD_READ_SINGLE_BLOCK,sector);     
   if (r1) 
   {
     SPI_CS_HI;
 ...
 * sector - where to store in the card (address has to be sector aligned)
 *
 * RETURNS
 *
 * 0 - if successful
 ...
 ***************************************************************************/
#if ASM_OPT_8051==0
unsigned char mmc_writesector (unsigned char *dwaddr,unsigned long sector) 
{
#if USE_CRC || SDHC_ENABLE
   ...
   sector<<=9;
#endif
   r1=mmc_cmd(CMD_WRITE_SINGLE_BLOCK,sector);
   if (r1) 
   {
     SPI_CS_HI;
     if (r1&BIT_IDLE_STATE) mmc_dsc.initok=0;
 ...
 * _get_csd_bits
 *
 * Gets specified bits from bitspace mmc_dsc.CSD
 ...
 *
 ***************************************************************************/
unsigned long _get_csd_bits (unsigned char st, unsigned char len)
{
  unsigned long dst;
  unsigned char *src=mmc_dsc.CSD;
...
static unsigned char mmc_getcsd (void) 
{
  unsigned char a;
  ...
  for (a=0;a<16;mmc_dsc.CSD[a++]=spi_rx1()); 
  SPI_CS_HI;
 ...
 #endif
  {
     ...
     if ((crc&0x7f)!=(mmc_dsc.CSD[15]>>1)) return MMC_ERR_CRC;
  }
#endif

  mmc_dsc.TRANSPEED=_get_csd_bits(96,8);
  
  if ((mmc_dsc.cardtype&_T_SDHC)==0)
  {
    mmc_dsc.R_BL_LEN=_get_csd_bits(80,4);
    mmc_dsc.CSIZE=_get_csd_bits(62,12);
    mmc_dsc.CSIZE_M=_get_csd_bits(47,3);
    mmc_dsc.NSAC=_get_csd_bits(104,8);
    mmc_dsc.TAAC=_get_csd_bits(112,8);
    mmc_dsc.R2W=_get_csd_bits(26,3);
  ...
  {
    mmc_dsc.number_of_sectors=_get_csd_bits(48,22)*1024;
  }
 ...
 *
 ***************************************************************************/
unsigned char mmc_initcard(void) 
{
   unsigned long a;
   unsigned char ret;
   unsigned char r1;
   ...
   for (a=0; a<1000; a++) spi_tx1(0xff); /* wait some */
 ...
 ***************************************************************************/
#if F_FORMATTING
unsigned char mmc_getphy (F_PHY *phy) 
{
