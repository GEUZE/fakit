 ...
 * Modifications : 
 * - Removal of dependency on lpc177x_8x_timer.h. 
 *   Required function, TIM_Waitus, is implemented directly in the file instead 
 *   (copied from DUT12, commit hash: 13b485b83208b3d3bfd73f4d5918dd345ea7fffa)
 * - Addition of LPC407x_8x_177x_8x.h header. Needed for implementation of TIM_Waitus
...
void TIM_Waitus(uint32_t time) {
	...
	LPC_TIM0->MR0 = time;
	/* LPC_TIM0->MCR &= ~7;
	LPC_TIM0->MCR |= 1 << 0; // Int on match
	LPC_TIM0->MCR |= 1 << 1; // Reset on match
	LPC_TIM0->MCR |= 1 << 2; // Stop on match */
	LPC_TIM0->MCR |= 7; // Int, reset and stop on match.
	LPC_TIM0->EMR &= ~(3 << 4); // EM mask
	//LPC_TIM0->EMR |= 0;
	LPC_TIM0->TCR	|= 1; // Enable
	while (!(LPC_TIM0->IR & 0x01)); // Wait for interrupt flag
	LPC_TIM0->TCR |= 2; // Reset flag
	LPC_TIM0->TCR &= ~2;
}
